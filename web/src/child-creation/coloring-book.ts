/**
 * 涂色本线稿：海里的小鱼 / 海龟 / 海豚仍用这里。
 * 陆地狮 / 鹿 / 虎的选一只、涂色、打印线稿改走官方 PNG，不要再画椭圆雪人。
 */
...
export const TIGER_OUTLINE: Ring = [
  ...
  [0.78, 0.72],
  ...pawLoop(0.58, 0.68, 0.09),
  [0.4, 0.66],
  ...pawLoop(0.3, 0.66, 0.085),
  [0.12, 0.62],
  [-0.18, 0.6],
  ...pawLoop(-0.4, 0.66, 0.09),
  [-0.56, 0.66],
  ...pawLoop(-0.72, 0.68, 0.09),
  [-0.86, 0.82],
  [-0.94, 0.96],
  [-1.08, 1.14],
  [-1.22, 1.24],
...
export function drawTigerRegions(ctx: Ctx): void {
  fillId(ctx, 11, () => stadium(ctx, -0.72, 0.04, 0.74, 0.09))
  fillId(ctx, 12, () => stadium(ctx, -0.4, 0.04, 0.72, 0.088))
  fillId(ctx, 9, () => stadium(ctx, 0.3, 0.04, 0.72, 0.085))
  fillId(ctx, 10, () => stadium(ctx, 0.58, 0.04, 0.74, 0.09))
  fillId(ctx, 13, () => {
    ctx.beginPath()
...
export function drawDeerRegions(ctx: Ctx): void {
  fillId(ctx, 14, () => stadium(ctx, -0.72, 0.04, 0.76, 0.07))
  fillId(ctx, 15, () => stadium(ctx, -0.4, 0.04, 0.74, 0.068))
  fillId(ctx, 12, () => stadium(ctx, 0.16, 0.04, 0.74, 0.068))
  fillId(ctx, 13, () => stadium(ctx, 0.48, 0.04, 0.76, 0.07))
  ...
  fillId(ctx, 4, () => oval(ctx, 1.06, 1.56, 0.11, 0.14))
...
export const LION_OUTLINE: Ring = [
  ...
  [0.34, 0.68],
  ...pawLoop(0.22, 0.68, 0.085),
  [0.04, 0.64],
  [-0.22, 0.62],
  ...pawLoop(-0.4, 0.68, 0.09),
  [-0.56, 0.68],
  ...pawLoop(-0.72, 0.7, 0.092),
  [-0.86, 0.84],
  [-1.08, 0.92],
  [-1.28, 0.78],
  [-1.4, 0.86],
...
export function drawLionRegions(ctx: Ctx): void {
  fillId(ctx, 10, () => stadium(ctx, -0.72, 0.04, 0.74, 0.09))
  fillId(ctx, 11, () => stadium(ctx, -0.4, 0.04, 0.72, 0.088))
  fillId(ctx, 8, () => stadium(ctx, 0.22, 0.04, 0.72, 0.085))
  fillId(ctx, 9, () => stadium(ctx, 0.5, 0.04, 0.74, 0.09))
  fillId(ctx, 12, () => {
    ctx.beginPath()
