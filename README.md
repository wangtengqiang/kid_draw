...
| 创作端 | 「开始画画」或链接 `?join=房号` | 三步：选一只 → 涂色 → **送进世界**。扫码进来不用输数字。成功后显示「送到啦」，可再画一只。 |
...
1. 标签 A：点「打开世界」，右侧会出现二维码和 4 位房号。
2. 标签 B：打开二维码指向的 `?join=房号`（等于扫码），会直接「选一只」，不用输数字。
3. 涂色后按超大按钮 **送进世界**。回到 A，动物带着你的颜色走进森林。可切换森林 / 雪原 / 海底。
